      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, sans-serif; }
          ...
          .avatar-circle { width: 45px; height: 45px; border-radius: 50%; background: var(--accent); display: flex; align-items: center; justify-content: center; font-weight: bold; flex-shrink: 0; }
      ...
      <body>
        ...
        <div id="app-interface">
          ...
          <div class="drawer" id="drawer">
            <div class="drawer-header">
              <div class="avatar-circle" id="p-avatar" style="margin-bottom:10px; border: 2px solid #fff">?</div>
          ...
          <div class="sidebar" id="sidebar">
            <div class="top-nav">
              <i class="fas fa-bars" onclick="toggleDrawer(true)" style="cursor:pointer; font-size: 1.2em;"></i>
          ...
          <div class="main-chat" id="chat-window">
            <div class="chat-head">
              <i class="fas fa-arrow-left" onclick="closeChat()" style="cursor:pointer; font-size: 1.2em;"></i>
              <div class="avatar-circle" id="t-avatar" style="width:35px; height:35px; font-size:0.8em; margin-left: 10px;">?</div>
        ...
        <script>
          ...
          socket.on('search_res', users => {
            ...
            list.innerHTML = '<div style="padding:10px; font-size:0.8em; color:gray">Глобальный поиск:</div>';
          ...
          function addChatUI(user, isSearch = false) {
            ...
            div.innerHTML = \`
              <div class="avatar-circle">\${user[1].toUpperCase()}</div>
              <div style="flex:1; overflow:hidden">
                <b>\${user}</b><br>
                <small style="color:gray" id="l-\${user}">Кликните, чтобы начать</small>
              </div>\`;
            div.onclick = () => { openChat(user); toggleDrawer(false); };
            const list = document.getElementById('chat-list-ui');
...
io.on('connection', s => {
  ...
  s.on('search', q => {
    const res = Array.from(occupiedNames.keys()).filter(n => n.includes(q));
    s.emit('search_res', res);
  });
  s.on('private_msg', d => {
