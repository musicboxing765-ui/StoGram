  ...
  res.send(`
    ...
    <html lang="ru">
      <head>
        ...
        <title>StoGram Mobile Fix</title>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
        <style>
          :root { 
            --bg: #0e1621; --side: #17212b; --text: #ffffff; --accent: #5288c1; 
            --drawer-bg: #1c242d; --input-bg: #242f3d; --msg-out: #2b5278; --msg-in: #182533; 
          }
          * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, sans-serif; }
          ...
          body { 
            ...
            height: 100vh; height: 100dvh; /* Используем динамическую высоту */
          ...
          .sidebar { 
            width: 100%; max-width: 380px; background: var(--side); 
          ...
          /* НИЖНЯЯ ПАНЕЛЬ С ФИКСОМ ОТСТУПА */
          .bottom-bar { 
            height: 70px; /* Немного увеличили высоту */
            background: var(--side); 
            border-top: 1px solid #000; 
            display: flex; justify-content: space-around; align-items: flex-start; /* Прижали к верху */
            padding-top: 10px;
            padding-bottom: env(safe-area-inset-bottom, 15px); /* Авто-отступ для Android/iOS */
          }
          .nav-item { text-align: center; color: #808d9a; cursor: pointer; font-size: 0.75em; flex: 1; }
          .nav-item.active { color: var(--accent); }

          .main-chat { flex: 1; display: none; flex-direction: column; background: var(--bg); position: relative; height: 100%; }
          ...
          .bubble { padding: 8px 14px; border-radius: 12px; max-width: 80%; position: relative; }
          ...
          .input-area { padding: 10px 15px; background: var(--side); display: flex; align-items: center; gap: 10px; padding-bottom: env(safe-area-inset-bottom, 10px); }
          ...
          @media (max-width: 768px) { 
            ...
            .main-chat.active { position: fixed; inset: 0; z-index: 100; } 
          }
        </style>
      </head>
      <body>
        ...
        <div id="app-interface">
          ...
          <div class="main-chat" id="chat-window">
            <div class="chat-head"><i class="fas fa-arrow-left" onclick="closeChat()"></i><div class="avatar" id="t-avatar" style="width:35px; height:35px; font-size:0.8em">?</div><div style="margin-left:10px"><b id="t-name"></b></div></div>
            <div id="messages"></div>
            <form class="input-area" id="msg-form"><input id="m-input" placeholder="Сообщение..." autocomplete="off"><button type="submit" style="background:none; border:none; color:var(--accent); font-size:1.4em;"><i class="fas fa-paper-plane"></i></button></form>
          </div>
        </div>

        <script src="/socket.io/socket.io.js"></script>
        <script>
          // Весь JS код остается таким же, как в прошлом ответе
          const socket = io();
          ...
          function addChatUI(user, isSearch = false) {
            ...
            div.innerHTML = \`<div class="avatar">\${user[1].toUpperCase()}</div><div style="flex:1; margin-left:10px"><b>\${user}</b><br><small id="l-\${user}" style="color:gray">Открыть чат</small></div>\`;
          ...
          function renderMessages() {
            ...
            box.innerHTML = '';
            if(history[activeChat]) history[activeChat].forEach(m => addMsgUI(m.text, m.type, m.time));
          }
...
http.listen(PORT, () => console.log('StoGram Fixed Ready'));
